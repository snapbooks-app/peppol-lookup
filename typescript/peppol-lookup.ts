 ...
 */
async function smlLookup(icd: string, identifier: string, smlDomain: string = SML_DOMAIN): Promise<string | null> {
    try {
 ...
 */
async function smpLookup(smpHostname: string, icd: string, identifier: string): Promise<string[]> {
        ...
        http.get(url, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    reject(new Error(`Failed to parse SMP response: ${(error as Error).message}`));
                }
            });
        }).on('error', (error) => {
