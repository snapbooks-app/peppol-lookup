...
def sml_lookup(icd : String, identifier : String, sml_domain : String = SML_DOMAIN) : String?
  begin
    ...
    Socket::Addrinfo.resolve(hostname, "http", Socket::Type::STREAM)
    hostname
  rescue Socket::Error
    nil
  end
...
def smp_lookup(smp_hostname : String, icd : String, identifier : String) : Array(String)
  # Construct SMP URL
  # Format: http://[SMP hostname]/[identifier scheme]::[participant identifier]
  participant_id = "#{icd}:#{identifier}"
  encoded_id = URI.encode_www_form(participant_id)
  ...
  unless response.success?
    raise "Failed to fetch SMP data: HTTP #{response.status_code}"
  end

  # Extract document types from ServiceMetadataReference href attributes
  document_types = [] of String

  # Match ServiceMetadataReference href attributes
  response.body.scan(/ServiceMetadataReference[^>]*href="([^"]*)"[^>]*>/) do |match|
    href = URI.decode_www_form(match[1])
...
begin
  ...
  puts "- Supports Invoice" if document_types.includes?(BIS_BILLING_INVOICE)
  puts "- Supports Credit Note" if document_types.includes?(BIS_BILLING_CREDITNOTE)
